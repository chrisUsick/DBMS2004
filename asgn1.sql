...
-- Question 2
SELECT Name, TO_CHAR(Released, 'yyyy') AS "Released In", 
...
--    C)
SELECT Min(LENGTH(Name)) AS "Length"
FROM Movie;

SELECT MAX(LENGTH(Name)) AS "Length"
FROM Movie;

SELECT AVG(LENGTH(Name)) AS "Length"
FROM Movie;

-- Question 7 
SELECT TO_CHAR(AVG(RentalAmount), '$9.99') AS "Average"
FROM RentalAgreement NATURAL JOIN MovieRented;

-- Question 8 
SELECT REPLACE(Name, ': ', CHR(10)) || CHR(10)
FROM Movie
WHERE Name LIKE '%: %';
...
REFRESH COMPLETE
AS SELECT Name, S.GenreType, S.Stars
